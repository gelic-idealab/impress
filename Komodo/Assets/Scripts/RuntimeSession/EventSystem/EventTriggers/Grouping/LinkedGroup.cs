...
{
    public  List<Collider> uniqueIdToParentofLinks;
    ...
    public void RefreshLinkedGroup()
    {
        ...
        if(uniqueIdToParentofLinks.Count != 0 && uniqueIdToParentofLinks.Count != 0)
        newBound = new Bounds(uniqueIdToParentofLinks[0].transform.position, Vector3.one * 0.02f);// new Bounds();
